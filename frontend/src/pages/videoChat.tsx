...
export default function VideoChat() {
  ...
  <div className="flex flex-col md:flex-row items-center gap-6">
    <video
      ...
      autoPlay
      playsInline
      className="w-full md:w-96 h-64 rounded-3xl border border-gray-300 shadow-lg transition-transform duration-300 ease-in-out hover:scale-[1.05] hover:shadow-2xl"
    />
    <video
      ref={remoteVideoRef}
      autoPlay
  ...
  <div>
    {!inCall ? (
      <button
        onClick={startCall}
        className="bg-green-600 text-white px-10 py-4 rounded-2xl font-semibold hover:bg-green-700 shadow-md transition"
      >
        Démarrer l'appel
      </button>
    ) : (
      <button
        onClick={endCall}
        className="bg-red-600 text-white px-10 py-4 rounded-2xl font-semibold hover:bg-red-700 shadow-md transition"
      >
        Terminer l'appel
      </button>
    )}
